...
class Vector3 {
	static FlattenArray(arr: Vector3[])
	{
		let res: number[] = [];
	...
	static VectorsFromArray(arr : ArrayLike<number>)
	{
		let res: Vector3[] = [];
		for (var i = 0; i < arr.length / 3; i++)
		{
			res.push(new Vector3(arr[(i * 3) + 0], arr[(i * 3) + 1], arr[(i * 3) + 2]));
		}
		return (res);
	}
	...
	angleTo(v) {
		...
		return Math.acos(MathHelper.Clamp(theta, - 1, 1));
