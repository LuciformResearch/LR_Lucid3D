...
class Quaternion {
	...
	lookRotation(forward: Vector3, upward: Vector3, normalize: boolean = true)
	{
		return (QuaternionHelper.LookRotation(forward, upward, normalize, this));
	}
	...
	angleTo(q) {
		...
		return 2 * Math.acos(Math.abs(MathHelper.Clamp(this.dot(q), - 1, 1)));
